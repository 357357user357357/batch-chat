...
function buildHtml(
  ...
  return [
    ...
    "          window.ReactNativeWebView.postMessage('mathjax:error:' + message);",
    "        }",
    "      };",
    "      window.MathJax = {",
    "        tex: {",
    ...
    "        // MathJax mutates already-painted text nodes into SVG; some WebView",
    "        // compositors (seen on MediaTek/MIUI devices) leave stale rasterized",
    "        // tiles for the part of a tall page painted before the mutation, so",
    "        // an opacity nudge alone isn't enough — force a full reflow by",
    "        // toggling display (reading offsetHeight flushes the layout).",
